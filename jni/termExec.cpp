...
static int create_subprocess(const char *cmd, const char *arg0, const char *envir, const char *dir,
    int* pProcessId)
{
    char *devname;
    int ptm;
    pid_t pid;
    if (envir!=NULL) putenv(envir);//further checking would be nice but this works for now
    if (dir!=NULL) chdir(dir);//further checking would be nice but this works for now
...
static jobject android_os_Exec_createSubProcess(JNIEnv *env, jobject clazz,
    jstring cmd, jstring arg0, jstring arg1, jstring arg2, jintArray processIdArray)
{
    ...
    int procId;
    int ptm = create_subprocess(cmd_8.string(), arg0Str, arg1Str, arg2Str, &procId);
...
static const char *classPathName = "de/moliso/shelxle/Exec";
